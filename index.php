...
if($_GET['do'] == 'list') {
    ...
    // listen to the rename event
        ...
        if(file_exists(realpath($file))){
            ...
            rename(realpath($file),$base_path.$new_name);
        }
}
...
.rename{
    background: url('data:image/png;base64,') no-repeat scroll 0px 5px;
    padding:4px 0 4px 20px;
    margin-left: 15px;
    color: darkorange;
    cursor: pointer;
...
<?php endif; ?>
	...
	function renderFileRow(index,data) {
		...
		var $rn_link = $('<a/>')
            .attr('id','rename-' + index)
            .attr('role','button')
            .click(function(){
                rename_file('rename-' + index,data.name,encodeURIComponent(data.path))
            })
    ...
    //show input edit to rename
	function rename_file(id,old_name,path){
        var split_id = id.split("-")
        var edit_input = document.createElement("input")
        edit_input.setAttribute('id',id)
        edit_input.setAttribute('placeholder',old_name)
        document.getElementById('name-' + split_id[1]).replaceWith(edit_input)

        //disable buttons
        var btn_delete = document.getElementsByClassName('delete');
	    var btn_download = document.getElementsByClassName('download');
	    var btn_rename = document.getElementsByClassName('rename');

        for (var i = 0; i < btn_delete.length; i++) {
            btn_delete[i].classList.toggle('isDisabled')
        }
        for (var i = 0; i < btn_download.length; i++) {
            btn_download[i].classList.toggle('isDisabled')
        }
        for (var i = 0; i < btn_rename.length; i++) {
            btn_rename[i].classList.toggle('isDisabled')
        }

        //get parent of input node, insert save button
        var node_father_input = document.getElementById('rename-' + split_id[1]).parentNode
        var btn_save = document.createElement('button')
        var btn_cancel = document.createElement("button")

        btn_save.append('save')
        btn_save.setAttribute('id','saved-'+ split_id[1])
        btn_save.classList.add('btn_save')
        btn_cancel.append('cancel')
        btn_cancel.classList.add('btn_cancel')


        btn_save.addEventListener("click", function () {
            saved_rename(path,edit_input.value)
        })
        ...
        node_father_input.append(btn_save)
        node_father_input.append(btn_cancel)
    }

    function saved_rename(old_name,new_name) {
        console.log(old_name,new_name)
        location.href="./?do=rename&file="+old_name+"&newname="+new_name
    }
})
